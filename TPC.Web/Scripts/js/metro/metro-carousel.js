    ...
    $.widget("metro.carousel", {
        ...
        options: {
            ...
            markers: {
                show: false,
                type: 'default',
                position: 'bottom-center' //bottom-left, bottom-right, bottom-center, top-left, top-right, top-center
            },
            controls: true,
            stop: true,
            width: '700px',
        ...
        _create: function(){
            ...
            //added by chandana for intital the hiding the left arrow 
            //carousel.find('.controls.left').hide();
            if (this.options.controls && this._slides.length > 1) {
                carousel.prev('div').find('.controls.left').on('click', function(){
                    that._slideTo('prior');
                });
                carousel.next('div').find('.controls.right').on('click', function () {
                    that._slideTo('next');
                });
            } else {
                controls.hide();
        ...
        _slideTo: function(direction){
            ...
            if (direction === 'prior') {
                ...
                //added by chandana for stoping the hiding the arrows if the starting point is reached
                if (this._currentIndex == 0) carousel.prev('div').find('.controls.left').addClass('hide');
                carousel.next('div').find('.controls.right').show();
                //
                if (this._currentIndex < 0) this._currentIndex = this._slides.length - 1;
            
                this._outPosition = this.element.width();
                carousel.next('div').find('.controls.right').removeClass('hide');

            } else if (direction === 'next') {
                this._currentIndex += 1;
                //added by chandana for stoping the hiding the arrows if the ending point is reached
                carousel.prev('div').find('.controls.left').show();
                if (this._currentIndex == this._slides.length - 1) carousel.next('div').find('.controls.right').addClass('hide');
                //
                if (this._currentIndex >= this._slides.length) { this._currentIndex = 0; }
               
                this._outPosition = -this.element.width();
                carousel.prev('div').find('.controls.left').removeClass('hide');
            }
        ...
        _markers: function (that) {
            ...
            for (i = 0; i < this._slides.length; i++) {
                li = $('<li><a href="javascript:void(0)" data-num="' + i + '"><button style="border-radius: 100%; height: 25px; width: 25px;background-color:#00a0cf"></button></a></li>');
                if (i === 0) {
                    li.addClass('active');
                }
                li.appendTo(ul);
