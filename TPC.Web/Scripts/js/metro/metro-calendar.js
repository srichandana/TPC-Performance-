...
(function ($) {
    $.widget("metro.calendar", {
        ...
        _renderMonth: function () {
            ...
            //added by chandana for future Billing Date of Submit Quote--Start
            var curentDay = this._today.getDate(),
                currentMonth = this._today.getMonth(),
                currentYear = this._today.getFullYear();


            var myDate = new Date();
            myDate.setDate(myDate.getDate() + 90);
            var futureDate = new Date(myDate.getFullYear(), myDate.getMonth(), myDate.getDate());
            //added by chandana for future Billing Date of Submit Quote--End
            if (month == 1) {
            ...
            tr = $("<tr/>");
            //added by chandana for future Billing Date of Submit Quote--Start
            var curclasshide = (month == currentMonth) == true ? "" : "<a class='btn-previous-month' href='#'><i class='icon-arrow-left-4 ' ></i></a>";
            var futclasshide = (month == futureDate.getMonth()) == true ? "" : "<a class='btn-next-month ' href='#'><i class='icon-arrow-right-4 ' ></i></a>";
            //added by chandana for future Billing Date of Submit Quote--End.

            $("<td/>").addClass("text-center").html("<a class='btn-previous-year hide' href='#'><i class='icon-previous'></i></a>").appendTo(tr);
            $("<td/>").addClass("text-center").html(curclasshide).appendTo(tr);
            $("<td/>").attr("colspan", 3).addClass("text-center").html("<a class='btn-select-month' href='#'>" + this.options.months[month] + ' ' + year + "</a>").appendTo(tr);
            $("<td/>").addClass("text-center").html(futclasshide).appendTo(tr);
            $("<td/>").addClass("text-center").html("<a class='btn-next-year hide' href='#'><i class='icon-next'></i></a>").appendTo(tr);
            ...
            for (i = 1; i <= daysInMonth; i++) {
                ...
                //added by chandana for future Billing Date of Submit Quote--Start
                if (month == futureDate.getMonth()) {
                    if (i > futureDate.getDate()) {
                        td = $("<td/>").addClass("text-center day emptytd").html(i);
                    }
                    else {
                    ...
                    if (year == this._today.getFullYear() && month == this._today.getMonth() && this._today.getDate() > i) {
                        td = $("<td/>").addClass("text-center day").html('');
                    }
                    else {
                        td = $("<td/>").addClass("text-center day").html("<a href='#'>" + i + "</a>");
                    }
        ...
        _renderMonths: function () {
            ...
            var futuremonth = myDate.getMonth();
            //added by chandana for future Billing Date of Submit Quote--End.
            ...
            //added by chandana for future Billing Date of Submit Quote--Start
            var curclasshide = (month == currentMonth) == true ? "" : "<a class='btn-previous-year' href='#'><i class='icon-arrow-left-4 ' ></i></a>";
            var futclasshide = (month == futureDate.getMonth()) == true ? "" : "<a class='btn-next-year hide' href='#'><i class='icon-arrow-right-4 ' ></i></a>";
            //added by chandana for future Billing Date of Submit Quote--End.
           ...
           // $("<td/>").addClass("text-center").html("<a class='btn-next-year ' href='#'><i class='icon-arrow-right-4'></i></a>").appendTo(tr);
            ...
            for (i = 0; i < 12; i++) {
                if (i <= futuremonth) {
                    ...
                    td = $("<td/>").attr('style', 'padding-top:20px;padding-bottom:20px').addClass("text-center month").html(this.options.monthsShort[i]);
                }
        ...
        _renderYears: function () {
            ...
            var futuremonth = myDate.getFullYear();
            //added by chandana for future Billing Date of Submit Quote--End.
            ...
            $("<td/>").addClass("text-center").html("<a class='btn-previous-year hide' href='#'><i class='icon-arrow-left-4'></i></a>").appendTo(tr);
            // $("<td/>").attr("colspan", 2).addClass("text-center").html((this._distance) + "-" + (this._distance + 11)).appendTo(tr);
            $("<td/>").attr("colspan", 2).attr('style', 'padding-top:5px;padding-bottom:5px').addClass("text-center").html((this._distance)).appendTo(tr);
            $("<td/>").addClass("text-center").html("<a class='btn-next-year hide' href='#'><i class='icon-arrow-right-4'></i></a>").appendTo(tr);
            ...
            for (i = this._distance; i < this._distance + 12; i++) {
                ...
                if (i == futuremonth) {
                    ...
                    td = $("<td/>").attr('style', 'padding-top:20px;padding-bottom:20px').addClass("text-center year").html(i);
                }
        ...
        _initButtons: function () {
            ...
            if (this._mode == 'day') {
                ...
                table.find('.btn-previous-month').on('click', function (e) {
                    ...
                    //commented  Previous Lines
                    //added by chandana for future Billing Date of Submit Quote--Start
                    //var myDate = new Date();
                    //if (that._month >= myDate.getMonth()) {
                    //    that._renderCalendar();
                    //}
                    //added by chandana for future Billing Date of Submit Quote--End

                    //Uncommented Bellow Line
                    that._renderCalendar();
                });
                table.find('.btn-next-month').on('click', function (e) {
                    ...
                    //myDate.setDate(myDate.getDate() + 90);
                    //var futureDate = new Date(myDate.getFullYear(), myDate.getMonth(), myDate.getDate());
                    //if (that._month <= futureDate.getMonth()) {
                    //    that._renderCalendar();
                    //}
                    //else {
                    //    if (that._month == 12) {
